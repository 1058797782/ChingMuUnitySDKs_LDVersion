...
public static class CMVrpn
{
    ...
    /// <summary>
    /// 通过刚体名称追踪数据的空间位置Position
    /// Get the Position of tracker Body
    /// </summary>
    /// <param name="address">ServerIP，for example:"MCAvatar@192.168.3.178" or "MCAvatar@SH1DT010"</param>
    /// <param name="channel">ID of Body</param>
    /// <returns></returns> 
    public static Vector3 CMPosByName(string address, int channel)
    {
        return new Vector3(
        (float)CMTrackerNoPredictExtern(address, channel, 0) / 1000f,
        (float)CMTrackerNoPredictExtern(address, channel, 2) / 1000f,
    ...
    /// <param name="lockYRotation">if value is true,the Y axis of rotation  will be lock </param>
    /// <returns></returns>
    public static Quaternion CMQuatByName(string address, int channel, bool lockYRotation = false)
    {
        return new Quaternion(
        (float)CMTrackerNoPredictExtern(address, channel, 3),
        (float)CMTrackerNoPredictExtern(address, channel, 5),
